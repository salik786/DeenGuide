...
export default async function ConversationPage({
  ...
  if (!isDbConfigured) {
    return (
      <div className="flex min-h-dvh items-center justify-center bg-cream px-4">
        <p className="text-sm text-emerald-900/60">Database not configured.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-dvh bg-cream px-4 py-8 sm:px-8">
      <div className="mx-auto max-w-2xl">
        ...
        <p className="mt-1 text-xs text-emerald-800/50">{messages.length} message{messages.length === 1 ? "" : "s"}</p>
              ...
              return (
                <div key={m.id} className="space-y-2">
                  ...
                  <div className="flex justify-start">
                    <div className="max-w-[90%] rounded-2xl rounded-tl-sm border border-emerald-900/10 bg-white px-4 py-3 text-sm shadow-sm">
                      <div className="mb-2 flex items-center gap-2">
                        ...
                        <span className="text-xs text-emerald-800/40">{formatTime(m.createdAt)}</span>
                      </div>
                      ...
                      {(m.citations?.length > 0 || m.webSources?.length > 0) && (
                        <div className="mt-3 space-y-1 border-t border-emerald-900/10 pt-2">
                          {m.citations?.map((c, i) => (
                            <a
                              key={`c${i}`}
                              href={c.url}
